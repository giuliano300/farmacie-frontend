...
export class BatchesComponent {
    ...
    get totalProductsToday(): number {
        return this.batch.reduce((total, item) => total + (item.report?.totalProducts ?? 0), 0);
    }

    get errorsToday(): number {
        return this.batch.reduce((total, item) => total + (item.report?.errors ?? 0), 0) +
            this.dashobardItem.reduce((total, item) => total + this.getBatchErrors(item), 0);
    }

    get successRate(): number {
        const reportedBatches = this.batch.filter(item => item.report);
        if (!reportedBatches.length) return 100;

        const successful = reportedBatches.filter(item => (item.report?.errors ?? 0) === 0).length;
        return Math.round((successful / reportedBatches.length) * 100);
    }
    ...
    get problematicActiveBatches(): BatchDashboardItem[] {
        return this.dashobardItem.filter(item => this.getBatchErrors(item) > 0);
    }

    get failedClosedBatches(): CompleteBatchesItem[] {
        return this.batch.filter(item => (item.report?.errors ?? 0) > 0);
    }
    ...
    getBatchErrors(batch: BatchDashboardItem): number {
        return (batch.heronImport?.errors ?? 0) +
            (batch.farmadati?.errors ?? 0) +
            (batch.suppliers?.errors ?? 0) +
            (batch.magento?.insertProducts?.errors ?? 0) +
            (batch.magento?.updateProducts?.errors ?? 0) +
            (batch.magento?.insertImages?.errors ?? 0);
    }
